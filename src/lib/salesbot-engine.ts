...
export class SalesBotEngine {
  ...
  async loadProducts(): Promise<void> {
    try {
      ...
      this.allProducts = snapshot.docs.map(
        (doc) =>
          ({
            id: doc.id,
            ...doc.data(),
  ...
  async evaluateAndGetMessage(): Promise<SalesBotMessage | null> {
    ...
    const cartAbandonment = this.checkCartAbandonment();
    if (cartAbandonment) messages.push(cartAbandonment);
    ...
    const newMessages = messages.filter(
      (msg) => !this.shownMessages.has(msg.message)
    );
  ...
  private checkCategoryInterest(): SalesBotMessage | null {
    ...
    const message = fillMessage(
      SALESBOT_MESSAGES.CATEGORY_INTEREST.WITH_ALTERNATIVES,
      {
  ...
  private checkEmptyCart(): SalesBotMessage | null {
    ...
    const suggestion =
      this.allProducts.find(
        (p) =>
          p.category === topCategory && p.stock > 0 && p.badge === 'Premium'
  ...
  // RULE: Cart Abandonment Recovery (PRIORITY: 80)
  private checkCartAbandonment(): SalesBotMessage | null {
    // Check if there are items in cart and user has been inactive
    if (this.context.cartItems.length === 0) return null;

    // This rule is triggered by the component after detecting inactivity
    // The actual message generation is done in salesbot-cart-recovery.ts
    return null; // Handled by SalesBotV2 component directly
  }
