...
import { trackSalesBotEvent } from '@/lib/salesbot-analytics';
...
export function SalesBotV2() {
  ...
  const [currentMessage, setCurrentMessage] = useState<SalesBotMessage | null>(
    null
  );
        ...
        if (
          ...
          if (days > 1 && days < 7) {
            setTimeout(() => {
              setCurrentMessage({
                message:
                  '¡Qué bueno verte de nuevo! Tenemos cortes frescos hoy.',
                icon: 'trending',
  ...
  const showMessage = (message: SalesBotMessage) => {
    ...
    // ✅ Track impression
    trackSalesBotEvent(message, 'impression');
  ...
  const handleAction = useCallback(
    (actionType: string) => {
      // ✅ Track action click
      if (currentMessage) {
        trackSalesBotEvent(currentMessage, 'click');
      }
      ...
      if (actionType === SALESBOT_ACTIONS.ADD_TO_CART && botEngine) {
        ...
        if (context.currentProduct) {
          ...
          // ✅ Track conversion
          if (currentMessage) {
            trackSalesBotEvent(currentMessage, 'conversion');
          }
      ...
      if (actionType === SALESBOT_ACTIONS.CHECKOUT) {
        ...
        // ✅ Track conversion (going to checkout)
        if (currentMessage) {
          trackSalesBotEvent(currentMessage, 'conversion');
        }
    ...
    [botEngine, router, addToCart, currentMessage]
  );

  const dismiss = () => {
    // ✅ Track dismissal
    if (currentMessage) {
      trackSalesBotEvent(currentMessage, 'dismiss');
    }
    setIsVisible(false);
  };
  ...
  if (!isFeatureEnabled('salesBotV2') || !isVisible || !currentMessage)
    return null;
