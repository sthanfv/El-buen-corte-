...
export default function ExperienceCommentsSection() {
  ...
  if (loading)
    return (
      <div className="text-center py-20 animate-pulse text-gray-500 italic">
        Cargando experiencias de la comunidad...
      </div>
    );
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {comments.length === 0 && (
          <p className="text-muted-foreground italic col-span-2">
            Aún no hay experiencias publicadas. ¡Sé el primero!
          </p>
        )}
        {comments.map((comment) => (
          <Card
            key={comment.id}
            className="bg-secondary/5 border border-white/5 p-6 rounded-lg relative group hover:border-primary/30 transition-all duration-500"
          >
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <span className="font-bold block text-sm italic">
                  {comment.authorName}
                </span>
            ...
            <div className="bg-primary/10 text-primary text-[9px] font-black uppercase tracking-[0.2em] px-2 py-0.5 rounded inline-block mb-3 italic">
              {comment.category === 'service'
                ? 'Servicio Ventas'
                : comment.category === 'delivery'
                  ? 'Logística'
                  : 'Precios'}
            </div>
