...
type ToastItem = Required<Pick<ToastOptions, "id" | "title" | "message" | "type" | "duration">>;
...
/* Viewport + Card */
const ToastViewport: React.FC<{ toasts: ToastItem[]; onRemove: (id: string) => void }> = ({ toasts, onRemove }) => {
...
const ToastCard: React.FC<{ toast: ToastItem; onClose: () => void }> = ({ toast, onClose }) => {
  ...
  useEffect(() => {
    const enter = setTimeout(() => { setVisible(true); setIconVisible(true); }, 20);
    ...
    return () => { clearTimeout(enter); clearInterval(id); };
  }, [toast.duration, onClose]);

  const baseColor = toast.type === "success"
  ...
  const barColor = toast.type === "success" ? "bg-green-400" : toast.type === "warning" ? "bg-yellow-400" : toast.type === "error" ? "bg-red-400" : "bg-blue-400";
  const icon = toast.type === "success" ? <Check size={20} className="text-green-300" /> : toast.type === "warning" ? <AlertTriangle size={20} className="text-yellow-300" /> : toast.type === "error" ? <ShieldAlert size={20} className="text-red-300" /> : <Check size={20} className="text-blue-300" />;
  ...
  return (
    ...
    >
      <div className={`transition-transform duration-300 ${iconVisible ? "scale-100" : "scale-75 opacity-0"}`}>{icon}</div>
      <div className="flex-1">
        {toast.title && <div className="text-sm font-bold">{toast.title}</div>}
        <div className="text-sm opacity-90">{toast.message}</div>
      </div>
      <button onClick={() => { setVisible(false); setTimeout(onClose, 300); }} className="ml-4 hover:opacity-70">
        <X size={16} />
      </button>
      <div className={`absolute bottom-0 left-0 h-[3px] ${barColor} transition-all duration-75`} style={{ width: `${progress}%` }} />
    </div>
  );
};
