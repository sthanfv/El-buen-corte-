  ...
  const handleAction = async (
    ...
    try {
      ...
      if (type === 'ip') {
        ...
        body = {
          ip: id,
          reason: 'Abuso detectado desde el panel administrativo',
        };
      } else {
      ...
      <div className="p-8 space-y-8 bg-background min-h-screen text-foreground">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          ...
          <Card className="bg-white dark:bg-zinc-900 border border-black/5 dark:border-white/5 shadow-xl">
            <CardHeader className="border-b dark:border-white/5 bg-gray-50/50 dark:bg-black/20">
              <CardTitle className="text-xl font-black italic uppercase tracking-tighter flex items-center gap-2">
                <Check className="w-5 h-5 text-green-500" /> Solicitudes de
                reserva
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              {requests.map((req) => (
                <div
                  key={req.ip}
                  className="p-4 rounded-lg border border-white/5 bg-black/20 flex justify-between items-start"
                >
                  <div>
                  ...
                  <div className="flex gap-2">
                    {req.status === 'pending' && (
                      <>
                        <Button
                          ...
                          onClick={() =>
                            handleAction('request', req.ip, 'approve')
                          }
                        >
                        ...
                        <Button
                          ...
                          onClick={() =>
                            handleAction('request', req.ip, 'reject')
                          }
                        >
                    ...
                    <Button
                      ...
                      onClick={() => handleAction('ip', req.ip, 'ban')}
                    >
          ...
          <Card className="bg-white dark:bg-zinc-900 border border-black/5 dark:border-white/5 shadow-xl">
            <CardHeader className="border-b dark:border-white/5 bg-gray-50/50 dark:bg-black/20">
              <CardTitle className="text-xl font-black italic uppercase tracking-tighter flex items-center gap-2">
                <MessageSquare className="w-5 h-5 text-blue-500" /> Moderación
                de Comentarios
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              {comments.map((com) => (
                <div
                  key={com.id}
                  className="p-4 rounded-lg border border-white/5 bg-black/20 space-y-2"
                >
                  <div className="flex justify-between items-start">
                    ...
                    <div className="flex gap-2">
                      {!com.approved && (
                        <>
                          <Button
                            ...
                            className="border-green-500/50 hover:bg-green-500/20"
                            onClick={() =>
                              handleAction('comment', com.id, 'approve')
                            }
                          >
                            <Check size={14} className="text-green-500" />
                          </Button>
                          <Button
                            ...
                            className="border-red-500/50 hover:bg-red-500/20"
                            onClick={() =>
                              handleAction('comment', com.id, 'reject')
                            }
                          >
                            <X size={14} className="text-red-500" />
                          </Button>
                      ...
                      <Button
                        ...
                        variant="outline"
                        className="border-red-900/50 hover:bg-red-900/20"
                        onClick={() => handleAction('ip', com.ip, 'ban')}
                      >
                        <XOctagon size={14} className="text-red-600" />
                      </Button>
                    </div>
                  ...
                  <div className="flex gap-1">
                    {Array.from({ length: 5 }).map((_, i) => (
                      <span
                        key={i}
                        className={
                          i < com.rating ? 'text-yellow-500' : 'text-gray-600'
                        }
                      >
                        ★
                      </span>
            ...
            <div className="flex flex-wrap gap-2">
              {bannedIps.map((ip) => (
                <Badge
                  key={ip}
                  variant="outline"
                  className="border-red-900 text-red-700 font-mono italic"
                >
                  {ip}
                </Badge>
              ))}
              {bannedIps.length === 0 && (
                <p className="text-muted-foreground text-sm italic text-center w-full">
